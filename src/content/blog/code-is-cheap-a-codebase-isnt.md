...
And Uncle Bob tweeted that his ["current strategy is to not read any of the code written by my agents"](https://x.com/unclebobmartin/status/2080257779395154409). His argument: with enough constraints (unit tests, Gherkin, mutation testing, coverage) it's fine. Six weeks later he went further: he's ["starting to think that I'm over constraining them"](https://x.com/unclebobmartin/status/2096994914185662851).
...
Bob C. Martin charted the tar pit in the first chapter of [*Clean Architecture*](https://www.informit.com/store/clean-architecture-a-craftsmans-guide-to-software-structure-9780134494166): productivity per engineer falling release after release, heading for zero, while the cost of every line climbs. He calls it the signature of a mess. I'd argue that chart is the reason the book was written. In a lot of ways that book is what the job of a software developer was to me: standing up for standards and architecture, and weighing short-term gain against long-term velocity on every change.
...
Everything in bucket one I've handed off. Bugs to the bot, "is the direction right" to the planning session, SOC 2 to the auditors until the control changes.

Everything in bucket two is my main focus, and the part where I keep looking for ways to replace myself, with more rules, hooks, whatever works. Not what I can verify today. What I pay for in month six, in the code and in the people working in it.

How would I even know? With one team we started measuring: output, velocity, bug metrics, and a regular dev check-in. Compared to the days of no AI or autocomplete-style AI, velocity is up about 4x across everything we track. Fewer outages. Long-standing issues closed, tech debt squashed, and bugs under our zero-bug policy get fixed on average in less than eight hours instead of seven days. Very outcome-focused again, I know, and I still don't think it captures the essence. Can you really quantify quality? Maybe it doesn't matter. What those numbers do buy me is the right to be cutting edge instead of bleeding edge. At least while we're still figuring out the right path forward.

So, are PRs one of the places the lights stay on? For bucket one, no. Let the factory run. For bucket two, yes, at least for now, because the agent doesn't have to live with the code and the team does.
