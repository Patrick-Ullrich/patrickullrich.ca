...
**Units, not features.** Features run away from you. One giant grill-me session and then "build the feature" tends to produce the fifteen-PR stack because it was asked for a feature. I'm going back to something closer to how we worked early on with Cursor and Windsurf in the editor: one piece at a time. Build a unit, verify the interface, build the next. Slower per feature. Perhaps faster overall. Especially after a prototyping session or two first, so the units are the right ones. The trick is finding the threshold. Below it, small and low blast radius, the agent one-shots the change and I don't look twice. Above it, we hand-craft the shape and let the agent fill it in. I don't want to over-correct here. I know I'm the bottleneck. I'm hoping to find the balance.
